...
public final class UtilNumber {
   ...
   private static  Logger log     = LoggerFactory.getLogger(UtilNumber.class);
    ...
    public static int getBigDecimalScale(String file, String property) {
        if (!file.isEmpty() || property.isEmpty()) {
        ...
        if (scale == -1) {
             log.warn("Could not set decimal precision from " + property + "=" + value + ". Using default scale of "
                    + DEFAULT_BD_SCALE + ".");
    ...
    public static String formatRuleBasedAmount(double amount, Locale locale) {
        ...
        if (ruleSet == null) {
             log.warn("Cannot format rule based amount for locale " + locale.toString()
                    + " because rule set for that locale does not exist");
            return "";
        }
    ...
    public static String formatRuleBasedAmount(double amount, String ruleSet, String rule, Locale locale) {
        ...
        try {
            ...
            log.error("Failed to format amount " + amount + " using rule " + rule, e);
        }
        return result;
    }
